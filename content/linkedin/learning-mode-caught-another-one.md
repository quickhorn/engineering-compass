...
The project is still in stealth, so I'll keep the details light. What I can share is what we're building technically and how Learning Mode keeps catching things I would have shipped without understanding.

## Teaching an AI to Arrange Stars

Each constellation starts as a cluster of stars in a default spiral layout. Functional, but every constellation looks the same. We wanted to give each one a visual identity, so we built a feature where we analyze our content, and prompt Claude's Haiku model to produce a visual identify of the content that then prompts Haiku again to arranges the stars into that shape.
...
3. The overall arrangement should look like a telescope
...
Stars use PixiJS pointer events for hover and click. Canvas panning uses DOM addEventListener on the same canvas element. Two completely separate event systems...clicking on a star fires both. We need a way to recognize a pan from a move, and to ensure both don't happen at the same time.
...
Read the full post with links and code formatting on my blog:
https://deegrey.com/blog/learning-mode-caught-another-one

Links referenced in this post:
- Learning Mode (my Goldilocks approach to AI-augmented learning): https://deegrey.com/blog/learning-mode-teaching-ai-to-teach
- Claude's Haiku model: https://docs.anthropic.com/en/docs/about-claude/models
- PixiJS (WebGL rendering library): https://pixijs.com/
